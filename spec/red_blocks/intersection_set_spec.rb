...
describe RedBlocks::IntersectionSet do
  let(:key1) { 'RB:Bar:1' }
  ...
  describe '#update!' do
    ...
    it 'stores proper data' do
      ...
      expect(RedBlocks.client.zrevrange(set.key, 0, -1)).to eq(['2', '3', RedBlocks.config.blank_id.to_s])
    end
    ...
    it 'stores on sum aggregator' do
      ...
      expect(RedBlocks.client.zrevrange(set.key, 0, -1, with_scores: true)).to eq([["2", 4.2], ["3", 4.0], [RedBlocks.config.blank_id.to_s, -RedBlocks.config.infinity * 2]])
    end
    ...
    it 'stores on min aggregator' do
      ...
      expect(RedBlocks.client.zrevrange(set.key, 0, -1, with_scores: true)).to eq([["2", 0.2], ["3", 0.0], [RedBlocks.config.blank_id.to_s, -RedBlocks.config.infinity]])
    end
